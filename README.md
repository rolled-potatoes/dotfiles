...
`install.sh`는 Ghostty와 GUI Neovim 설정에 사용하는 Cascadia Code와 D2Coding을 Homebrew cask로 설치한다. Cascadia Code는 우선 사용하고, 한글·기타 넓은 문자는 D2Coding으로 표시한다. 폰트 파일은 저장소에 포함하지 않는다.
...
- `brew list --cask font-cascadia-code font-d2coding`로 Cascadia Code와 D2Coding 설치를 확인한다.
...
- Cascadia Code와 D2Coding을 포함한 시스템 폰트 파일 (설치 스크립트가 Homebrew cask로 설치)
